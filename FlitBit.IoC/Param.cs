...
namespace FlitBit.IoC
{
	...
	public abstract class Param
	{
		...
		public bool HasName
		{
			get { return Kind.HasFlag(ParamKind.Named); }
		}
		...
		public static Param Named<T>(string name, T value) { return new ParamValueNamed<T>(ParamKind.UserSupplied | ParamKind.Named, name, value); }
		...
		/// <typeparam name="T">type T</typeparam>
		/// <returns>a param</returns>
		public static Param Resolve<T>() { return new ParamFromContainer<T>(); }
		...
		/// <returns>a param</returns>
		public static Param ResolveNamed<T>(string name) { return new ParamFromContainerNamed<T>(name); }
		...
		/// <param name="value">the value</param>
		/// <returns>a param</returns>
		public static Param FromValue<T>(T value) { return new ParamValue<T>(ParamKind.UserSupplied, value); }

		internal static Param Declared<T>(T value) { return new ParamValue<T>(ParamKind.DeclaredDefault, value); }

		internal static Param[] GetDefaultParamsUsingReflection(ConstructorInfo ci)
		{
			var defined = ci.GetParameters();
			var result = new Param[defined.Length];
			...
			for (var i = 0; i < defined.Length; i++)
			{
				...
				{
					result[i] = MakeValueUsingReflection(ParamKind.DeclaredDefault, p.ParameterType, p.DefaultValue);
				}
				else if (p.ParameterType.IsClass || p.ParameterType.IsInterface)
				{
					result[i] = MakeContainterSupplied(p.ParameterType);
				}
				else
				{
					result[i] = Missing(p.Name, p.Position, p.ParameterType);
				}
			}
			return result;
		}
		...
		{
			var tt = typeof(ParamFromContainer<>).MakeGenericType(type);
		...
		internal static Param MakeValueUsingReflection(ParamKind kind, Type type, object value)
		{
			...
			var tt = typeof(ParamValue<>).MakeGenericType(type);
			return (Param) Activator.CreateInstance(tt, kind, value);
		}

		internal static Param Missing(string name, int position, Type type) { return new ParamMissing(name, position, type); }
				...
				if (defaults.Length == defined.Length)
				{
					...
					for (var i = 0; i < defined.Length; i++)
					{
						if (defaults[i].Kind.HasFlag(ParamKind.Named)
							&& defaults[i].Name != defined[i].Name)
						{
							break;
						}

						if (!defined[i].ParameterType.IsAssignableFrom(defaults[i].TypeofValue))
						{
							break;
						}
